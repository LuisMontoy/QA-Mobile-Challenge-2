 ...
 */
async function allocateDevtoolsChannel(adb, socketName, webviewDevtoolsPort = null) {
...
const WebviewHelpers = {
    ...
    parseWebviewNames(webviewsMapping, { ensureWebviewsHavePages = true, isChromeSession = false } = {}) {
        ...
        for (const { webview, pages, proc, webviewName } of webviewsMapping) {
            if (ensureWebviewsHavePages) {
                if (lodash_1.default.isUndefined(pages)) {
                    logger_1.default.info(`Skipping the webview '${webview}' at '${proc}' since it is unreachable`);
                    continue;
                }
                if (!pages?.length) {
    ...
    async getWebViewsMapping(adb, { androidDeviceSocket = null, ensureWebviewsHavePages = true, webviewDevtoolsPort = null, enableWebviewDetailsCollection = true, waitForWebviewMs = 0, } = {}) {
        ...
        do {
            webviewsMapping = (await webviewsFromProcs(adb, androidDeviceSocket));
            if (webviewsMapping.length > 0) {
                break;
            }
            logger_1.default.debug(`No webviews found in ${timer.getDuration().asMilliSeconds.toFixed(0)}ms`);
